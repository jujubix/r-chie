...
plotArcs <- function(i, j, length, x = 0, y = 0, flip = FALSE, shape = "circle", ...) {
	i <- as.numeric(i)
	j <- as.numeric(j)
	for (n in 0:(length - 1)) {
...
plotHelix <- function(helix, x = 0, y = 0, flip = FALSE, line = FALSE, arrow = FALSE,
	...
	if (nrow(helix) > 0) {
		...
		for (n in nrow(helix):1) {
			if (!is.null(helix$cmd)) {
				...
				plotArcs(helix$i[n], helix$j[n], helix$length[n], x, y, flip = flip,
					shape = shape)
			}
...
plotOverlapCovariance <- function(predict.helix, known.helix, msa, bot.msa = TRUE,
	...
	plotCovariance(msa, known.helix, add = TRUE, arcs = FALSE, grid = grid, species = species, legend = legend, y = height/2, ...)
}
...
plotCovariance <- function(msa, helix, arcs = TRUE, add = FALSE, grid = FALSE,
		...
		shape = "circle", conflict.lty = 2, conflict.col = NA,
